...
class FaccioliEtAl2010Ext(CauzziFaccioli2008):
    """
    Modification of FaccioliEtAl2010 GMPE, extended to T=0.03 s
    by interpolating between the values for PGA and T=0.05 s.
    ...
    def _compute_mean(self, C, mag, dists, vs30, rake, imt):
        ...
        """
        if isinstance(imt, SA) and np.allclose(imt.period, 0.03):
            ## Take mean of PGA and SA(0.05)
            pga = PGA()
            mean_pga = self._compute_mean(self.COEFFS[pga], mag, dists, vs30, rake, pga)
            sa = SA(0.05, damping=imt.damping)
            mean_sa = self._compute_mean(C, mag, dists, vs30, rake, sa)
            mean = np.log((np.exp(mean_pga) + np.exp(mean_sa)) / 2.)
        else:
    ...
    ## Line for T=0.03 is a copy of line for T=0.05 !
    ...
    0.03      -2.9600   0.6040    -1.8780   0.0520    0.3960    0.2000    0.2100    0.1800    -0.0200   0.0800    -0.0300   0.3700
